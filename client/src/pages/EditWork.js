...
const EditWork = () => {
    ...
    return (<>
      <div className='form-container'>
        {work ? (<form className='form' onSubmit={handleSubmit}>
              <h1>Edit Work</h1>
              <label htmlFor='tName'>Work Name</label>
              <textarea id='tName' name='name' value={work.name || ''} onChange={handleTextAreaChange}/>
              <label htmlFor='tStatus'>Work Status</label>
              <select name='status' id='tStatus' value={work.status || ''} onChange={handleChange}>
                <option value='Todo'>Todo</option>
                <option value='In Progress'>In Progress</option>
                <option value='Done'>Done</option>
            </select>
            <label htmlFor='tDescription'>Work Description</label>
              <textarea id='tDescription' name='description' value={work.description || ''} onChange={handleTextAreaChange}/>
              <button type='submit'>Submit Form</button>
            </form>) : (<div>Issues fetching Work to Edit</div>)}
      </div>  
    </>);
};
